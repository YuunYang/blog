...
- 首先需要比较泛化性能，然而我们目前测得的是测试集上的性能，并不能代表泛化性能
- 即便是测试集上的性能也与测试集的选择、以及包含测试样例的不同而有较大的差异
- 机器学习算法的随机性导致即便上述条件都一样，最后也不一定得出相同的结果

统计假设检验（hypothesis test）为我们进行学习器性能比较提供了重要的依据。假设检验的出现就是解决上面所提出的疑问，如果观察到在测试集上学习器A的性能比B好，那么怎么知道A的泛化性能是否也比B好呢？下面介绍两种最基本的假设检验。
